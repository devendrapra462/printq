...
function App() {
  ...
  useEffect(() => {
    ...
    styleSheet.innerText = `
      ...
      .stat-num { font-family: 'Oswald', sans-serif; font-size: 2.2rem; color: #e6522b; line-height: 1; font-weight: 700; white-space: nowrap; }
      ...
      @media (max-width: 1024px) {
        .brutal-header { font-size: 5rem; }
        .outline-drop-text { font-size: 6rem; }
      }

      @media (max-width: 768px) {
        .grid-container { flex-direction: column; }
        .grid-left { border-right: none; border-bottom: 1px solid #000000; padding: 40px 20px; }
  ...
  const uploadFilesBatch = async (files) => {
    ...
    setUploadStatus('✅ FILES TRANSMITTED!');
    setUploadProgress(0);
  };
  ...
  return (
    <div onDragOver={(e) => e.preventDefault()} onDrop={(e) => { e.preventDefault(); if (e.dataTransfer.files) uploadFilesBatch(e.dataTransfer.files); }} style={{ width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#ebe6dd' }}>
      ...
      <header style={{ width: '100%', borderBottom: '1px solid #000000', padding: '20px 40px', boxSizing: 'border-box', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontFamily: 'Oswald', fontSize: '1.8rem', fontWeight: '700', textTransform: 'uppercase', color: '#000000' }}>PrintQ<span style={{ color: '#2b5ce6' }}>⚡</span></span>
        <span style={{ fontFamily: 'Oswald', fontSize: '1rem', color: '#706b64' }}>DEVELOPER HUB</span>
      </header>
      ...
      <div className="grid-container">
        ...
        <div className="grid-left">
          ...
          <div style={{ marginTop: '40px' }}>
            {isCustomer ? (
              ...
              <button onClick={() => window.location.reload()} className="btn-black">→ START NEW SESSION</button>
            )}
          </div>
        </div>
        ...
        <div className="grid-right">
          ...
          {/* QR/STATUS ZONE */}
          <div style={{ flex: '1', padding: '40px', borderBottom: '1px solid #000000', display: 'flex', flexDirection: 'column', justifyContent: 'center', boxSizing: 'border-box' }}>
            {isCustomer ? (
              <div style={{ textAlign: 'center' }}>
                <h2 className="outline-drop-text">READY</h2>
                <p style={{ fontFamily: 'Oswald', fontSize: '1.2rem', color: '#706b64', marginTop: '10px' }}>PIPE CONNECTED</p>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '30px', alignItems: 'center' }}>
                <div>
                  <h2 className="outline-drop-text">DROP</h2>
                  <p style={{ fontSize: '0.95rem', color: '#57534e', marginTop: '15px', lineHeight: '1.4' }}>
                    Point your smartphone camera at this code to securely couple devices. Drag files anywhere to test.
                  </p>
                </div>
                <div style={{ background: '#ffffff', padding: '16px', border: '1px solid #000000', boxShadow: '4px 4px 0px #000000', display: 'inline-block', height: '135px', width: '135px', boxSizing: 'content-box' }}>
              ...
              receivedFiles.map((file, index) => (
                <div key={index} style={{ border: '1px solid #000000', padding: '16px 20px', backgroundColor: '#ffffff', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', boxShadow: '3px 3px 0px #000000' }}>
                  <div style={{ overflow: 'hidden', marginRight: '15px' }}>
                    <p style={{ margin: '0 0 4px 0', fontWeight: '700', fontSize: '1.1rem', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>{file.fileName}</p>
                    <p style={{ margin: 0, fontSize: '0.75rem', fontWeight: '700', color: '#2b5ce6' }}>{file.fileType.split('/')[1] || 'ASSET'}</p>
                  </div>
                  <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                    <button onClick={() => triggerDirectPrint(file.fileData, file.fileName)} style={{ background: '#000000', color: '#ffffff', border: 'none', padding: '8px 16px', fontFamily: 'Oswald', cursor: 'pointer' }}>PRINT</button>
                    <button onClick={() => triggerManualDownload(file.fileData, file.fileName)} style={{ background: 'transparent', border: '1px solid #000000', padding: '8px 12px', cursor: 'pointer' }}>⬇</button>
                  </div>
                </div>
              ))
            )}
      ...
      <div style={{ padding: '30px 40px', boxSizing: 'border-box' }}>
        ...
        <div style={{ borderTop: '1px solid #000000', paddingTop: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.85rem', fontWeight: '600' }}>
          <span>© 2026 Devendra Developer</span>
